...
- (verify: the bundled test schema exposes no `Mutation` type, so mutation-specific behavior isn't directly exercised by tests; the laziness/forcing rules below are confirmed for queries and apply to mutations the same way)
...
- partial data with errors: TBD (no test exercises this; verify behavior against a server that returns `data` + `errors`)

// Package modref holds the pure, dependency-light logic for classifying and
// parsing module reference strings (local path vs git, scheme detection, git
// repo-root resolution). It is extracted verbatim from Dagger's core/gitref
// package (with its engine/vcs dependency vendored under ./vcs) so that Dang
// can resolve remote module refs using the exact same syntax and rules.
package modref
...
import (
	...
	"github.com/vito/dang/v2/pkg/modref/vcs"
)
...
func Parse(ctx context.Context, refString string) (_ Parsed, rerr error) {
	tracer := trace.SpanFromContext(ctx).TracerProvider().Tracer("github.com/vito/dang/v2/pkg/modref")
